...
def convert_single_episode(episode_dir, dataset, sample_data_list):
    ...
    all_rel_pose = cart_abs - start_pose
    ...
    # B. Action 是 "下一帧" 的 RelPose
    # Action[t] 应该预测 State[t+1] 将 Action 的定义从“上一帧的增量”改为“相对于本回合起点的位姿”。
    action_pos_target = np.zeros_like(cart_abs)
    action_pos_target[:-1] = all_rel_pose[1:] # 取下一帧
    action_pos_target[-1] = all_rel_pose[-1]  # 最后一帧保持
    # Action中的夹爪：预测下一步的绝对状态
    action_gripper = np.zeros_like(gripper)
    action_gripper[:-1] = gripper[1:]
    action_gripper[-1] = gripper[-1]
    # 拼接 Action
    # action_final_np = np.hstack([action_pos_delta, action_gripper]) #原先的相对于上一帧delta
    action_final_np = np.hstack([action_pos_target, action_gripper]) #现在的相对于起始点的delta
    # 打印信息
