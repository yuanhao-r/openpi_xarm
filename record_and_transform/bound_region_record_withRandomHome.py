...
class AutoDataRecorder:
    def __init__(self, ip, output_dir, camera_indices, crop_configs=None):
        ...
        self.pos_A = [554.626923, 361.343277, -70, 3.12897, 0.012689, -1.01436]
    ...
    #             self.pause_requested = True
    #             print("\n>>> [指令收到] 将在当前循环结束后暂停...")
    # 【新增】获取随机起始点 (Random Home)
    # ---------------------------------------------------------
    def get_random_start_pose(self):
        """
        在 pos_home 附近生成一个随机的起始点。
        """
        base_home = np.array(self.pos_home)
        
        # 随机偏移量
        # X: ±50mm, Y: ±100mm, Z: ±20mm
        # Yaw: ±0.2 rad (约11度)
        random_x = random.uniform(-100, 100)
        random_y = random.uniform(-100, 100)
        random_z = random.uniform(-20, 20)
        random_yaw = random.uniform(-0.2, 0.2)
        
        new_start_pose = base_home.copy()
        new_start_pose[0] += random_x
        new_start_pose[1] += random_y
        new_start_pose[2] += random_z
        new_start_pose[5] += random_yaw
        
        # Z轴安全检查
        # if new_start_pose[2] < 120.0:
        #      new_start_pose[2] = 100.0
             
        return new_start_pose.tolist()
    ...
    def run_auto_collection(self):
        ...
        try:
            while True:
                ...
                # 1. 生成本轮的随机起始点 (Random Home)
                current_random_home = self.get_random_start_pose()
                print(f"[Home] 随机起始点: ({current_random_home[0]:.1f}, {current_random_home[1]:.1f}, {current_random_home[2]:.1f})")
                ...
                # self.move_to(self.pos_home, speed=self.speed_fast)
                self.move_to(current_random_home, speed=self.speed_fast)
